...
def get_paragraphs(file_path):
    ...
    for i in range(1, 4):
        ...
        texts = []
        for p in paras:
            # We want to keep words together, especially drop caps
            # get_text without separator merges <span class="ap-lede">N</span>eonové -> Neonové
            text = p.get_text(separator="", strip=True)
            text = text.replace("\xa0", " ").replace("\n", " ").strip()
            # Still, there might be missing spaces between inline elements if not careful.
...
def align_robust(segments, paragraphs):
    ...
    for p in paragraphs:
        ...
        found = False
        for prefix_len in [8, 6, 5, 4, 3, 2, 1]:
            if len(words) < prefix_len: continue
            prefix = words[:prefix_len]
            # Use \W* instead of \W+ because whisper might have "na" and "vždy" as "navždy" etc
...
def main():
    ...
    results = {}
    for part in [1, 2, 3]:
        with open(f"scratch/dil_{part}_segments.json", "r", encoding="utf-8") as f:
            segments = json.load(f)
            
        cues = align_robust(segments, paragraphs[part])
